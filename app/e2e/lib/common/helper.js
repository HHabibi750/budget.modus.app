...
class helper {
    ...
    static waitForUrlToChange(urlShouldNotExit) {
        return browser.wait(async () => {
            ...
            // Commenting this out since it is causing a lot of logs.  We can turn ths back on
            // for debugging purposes, if needed.
            // console.log('current url', url, 'should not contain', urlShouldNotExit);
    ...
    static getTypeAndValue(field, options) {
        let type = field || field === '' ? '' : CONSTANTS[options.type.toUpperCase()];
        let value = field || field === '' ? field : options;
        return {
            type,
            value
        };
